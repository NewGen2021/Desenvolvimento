...
def home(request):
    ...
    contexto = {'lng': request.session.get('lng', 'pt'), 'grupo': grupo}
    return render(request, 'cliente_final/apresentacao/home.html', context=contexto)

def loginUser(request):
    contexto = {'lng': request.session.get('lng', 'pt')}
    return render(request, 'old/loginUser.html', context=contexto)
...
def agendamento(request, id_tipo_espaco):
    ...
    context = {
        ...
        'lng': request.session.get('lng', 'pt'),
    }
...
def reserva(request):
    if request.method == "POST":
        request.session['lng'] = 'en'
    contexto = {'lng': request.session.get('lng', 'pt')}
    return render(request, 'cliente_final/agendamento/reserva1EscolherEspaco.html', context=contexto)
...
def menuAdmin(request):
    if h.isAdministrator(request):
        ...
        context={'lng': request.session.get('lng', 'pt'), 'admInvalid': True}
        return render(request, 'cliente_final/apresentacao/index.html', context)
