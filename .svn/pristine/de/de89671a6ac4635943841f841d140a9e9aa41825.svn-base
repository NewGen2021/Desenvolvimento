...
def getDicionarioBusinessHours(diaSemana):
    for dicionario in businessHours:
        if diaSemana in dicionario['daysOfWeek']:
            return dicionario
    raise ValueError(
        'O diaSemana da getDicionarioBusinessHours tem que ser um número de 0 até 6. diaSemana informado = ' + str(
            diaSemana))
        ...
        for hora, valor in horarios.items():
            ...
            if comecoEvento == True and valor < vagas_max:
                ...
                for evento in event_list:
                    dataEvento = datetime.datetime.strptime(evento['start'], "%Y-%m-%dT%H:%M:%S").replace(hour=0,
                                                                                                          minute=0,
                                                                                                          second=0,
                                                                                                          microsecond=0)
                    dataAtual = datetime.datetime.strptime(data, "%Y-%m-%d").replace(hour=0, minute=0, second=0,
                                                                                     microsecond=0)
                ...
                for evento in reservasDoDia:
                    comecoReserva = datetime.datetime.strptime(evento['start'], "%Y-%m-%dT%H:%M:%S").replace(year=2020,
                                                                                                             month=1,
                                                                                                             day=1)
                    fimReserva = datetime.datetime.strptime(evento['end'], "%Y-%m-%dT%H:%M:%S").replace(year=2020,
                                                                                                        month=1, day=1)
                    ...
                    if comecoReserva <= comecoEventoLotado <= fimReserva:
...
def getReservasAtuais(id_tipo_espaco):
    return get_reservas_atuais(id_tipo_espaco=id_tipo_espaco)
...
def getReservasCliente(request, cpf_cnpj, id_tipo_espaco=None):
    if id_tipo_espaco:
        return get_reservas_cliente(cpf_cnpj=request.user.username, id_tipo_espaco=id_tipo_espaco)
    return get_reservas_cliente(cpf_cnpj=request.user.username)
