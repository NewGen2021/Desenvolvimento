...
def get_address_errors(address):
    ...
    if not address.isalnum():
        return 'Não são permitidos símbolos neste campo.'
    if not address.isalpha():
        return 'Não são permitidos números neste campo. Coloque o número do endereço no seu respectivo campo.'
    if contains_invalid_expression:
        return 'Confira se alguma letra está duplicada.'


def get_address_number_errors(address_number):
    address_number = str(address_number).strip()
    if len(address_number) > 5:
        return 'Números de endereço só podem ser até 5 caracteres.'
    if not address_number.isalnum():
        return 'Não são permitidos símbolos ou espaços no número do endereço.'
    if not address_number.isdigit():
        before_last_char = address_number[:-1]
        if not before_last_char.isdigit():
            return 'Colocar complementos no formato 111a, 111b.'
    return ''
...
def get_name_errors(name, pessoa=True):
    ...
    if not name.replace(' ', '').isalpha() and pessoa == True:
        return 'Certifique-se que o nome só contém letras. Números e símbolos não são permitidos.'
    if len(name) < 5:
        return 'Nome curto demais, colocar pelo menos 5 caracteres.'
    lista = name.split(' ')
    if len(lista) == 1 and pessoa:
        return 'Por favor coloque nome completo.'
