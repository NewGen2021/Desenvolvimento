...
class FormTesteForm(forms.ModelForm):
    ...
    class Meta:
        model= PagamentoModel
        fields= ["metodo", "cod_mercadopago", "status_pagamento", "id_reserva"]
...
class FormPagamento(forms.Form):
    ...
    mes_vencimento = forms.CharField(widget=forms.TextInput(
        attrs={'required': 'true', 'data-mask':"00"}), label=_("Mês vencimento"))
    ano_vencimento = forms.CharField(widget=forms.TextInput(
        attrs={'required': 'true', 'data-mask':"00"}), label=_("Ano vencimento"))
    nome_cartao = forms.CharField(widget=forms.TextInput(
        attrs={'required': 'true'}), label=_("Nome descrito no cartão"))
    email = forms.CharField(widget=forms.TextInput(
        attrs={'required': 'true'}), label=_("E-mail"))
    codigo_seguranca = forms.CharField(widget=forms.TextInput(
        attrs={'required': 'true', 'data-mask':"000"}), label=_("Código de segurança"))
