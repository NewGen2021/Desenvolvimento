...
@m.verificador()
@login_required
def agendamentoOld(request, context, id_tipo_espaco):
    ...
    for i in range(p.num_pages):
        ...
        pages.append(page)
    context['pages'] = pages
    context['tipo_espaco'] = EspacosModel.objects.filter(id_tipo_espaco=id_tipo_espaco)[0]
...
def agendamento(request, context, id_tipo_espaco):
    ...
    error = None
    if request.method == 'POST':
        dicionario = b_reserva.getDicionario(request)
        form = ReservaForm(copy.deepcopy(dicionario))
        if form.is_valid():
            hasErrors, error = b_reserva.getFormErrorsByApi(copy.deepcopy(dicionario), request, context)
            if not hasErrors:
                reserva = form.save()
                id_reserva = reserva.id_reserva
                return redirect('pagamento_reserva', id_reserva)
    ...
    context['error'] = error
    context['compartilhado'] = compartilhado
    context['pages'] = b_reserva.get_pages_of_tipo_espaco(id_tipo_espaco)
    context['tipo_espaco'] = EspacosModel.objects.filter(id_tipo_espaco=id_tipo_espaco)[0]
    return render(request, 'gere/template1/cliente/agendamento/reserva2Calendario.html', context)
...
def form_pagamento(request, context, id_reserva):
    ...
    if request.method == "POST":
        ...
        if mensagem_de_erro == '' or mensagem_de_erro == 'none':
            return redirect('listarReservasIndex')
        else:
            context['form'] = form
        if aluguel == 1:
            gerar_qrcode()
    ...
    context['valor_a_pagar'] = str(round(ReservaModel.objects.get(id_reserva=id_reserva).preco_total)).replace(',', '.')
    context['taxa_reserva'] = '0.1'
...
@m.verificador()
def tela_relatorio(request, context):
    return render(request, 'gere/template1/adm/menu_relatorios.html', context)
...
class ReservaList(ListView):
    ...
    def get_queryset(self):
        queryset = ReservaModel.objects.filter(data_reserva=datetime.date.today().strftime("%Y-%m-%d")).exclude(hora_saida_real__isnull=False)
        #queryset=ReservaModel.objects.all()
        return queryset

@m.verificador()
def finalizaReserva(request,context, id_reserva):
    ReservaModel.objects.filter(id_reserva=id_reserva).update(hora_saida_real = datetime.now().strftime("%H:%M:%S"))           
    
    return render(request, 'gere/template1/relatorios/reservas.html', context)
    
@m.verificador()
def iniciaReserva(request, context, id_reserva):
    ReservaModel.objects.filter(id_reserva=id_reserva).update(hora_entrada_real =datetime.now().strftime("%H:%M:%S"))
...
class PagamentoList(ListView):
    ...
    def get_queryset(self):
        queryset = ReservaModel.objects.all().exclude(id_pagamento= None).order_by('id_pagamento')
        return queryset

    def criarPDF():
        pdf = canvas.Canvas('Relatorio_Pagamentos.pdf')
        pdf.drawString(100,800, "Pagamentos Recebidos")
        pdf.save()

class AluguelList(ListView):
    model = ReservaModel
    template_name = 'gere/template1/relatorios/alugueis.html'
    paginate_by = 10

    def get_queryset(self):
        
        queryset = ReservaModel.objects.filter(is_aluguel = 1).order_by('data_reserva')

        return queryset

class Todas_Resrvas(ListView):
    model = ReservaModel
    template_name = 'gere/template1/relatorios/todas_reserva.html'
    paginate_by = 10

    def get_queryset(self):
        queryset = ReservaModel.objects.filter(is_aluguel = None).order_by('data_reserva')
        return queryset
...
class ComentarioList(ListView):
    ...
    model = ReservaModel
    template_name = 'gere/template1/funcionario/comentarios.html'

    def get_query(self):
        queryset = ReservaModel.objects.filter(data_reserva = datetime.date.today().strftime("%Y-%m-%d")).exclude(hora_saida_real = None)

        return queryset

@m.verificador()
def advertencia(request, context,id_reserva):
    if request.method == "POST":
         
        reserva = ReservaModel.objects.get(id_reserva=id_reserva)
        cpf_cnpj = (request.user.username)
        funcionario = FuncionariosModel.objects.get(cpf_cnpj=cpf_cnpj)
        comentario = request.POST['message']
        
        
        print(comentario)
        print(funcionario)
        AdvertenciasModel.objects.create( comentario=comentario, id_funcionario= funcionario, id_reserva=reserva)
        
        #AdvertenciasModel.objects.raw("INSERT INTO advertencias (comentario,id_funcionario,id_reserva) VALUES (%s, %s, %s )", [comentario, funcionario.id_funcionario, reserva.id_reserva])

        #AdvertenciasModel.objects.raw(f'INSERT INTO advertencias SET comentario="{comentario}", id_funcionario={funcionario.id_funcionario}, id_reserva={reserva.id_reserva});')
...
def testar_bootstrap(request, context):
    ...
    # context['url_imagem'] = TipoespacoModel.objects.get(id_tipoespaco=1).imagem.decode('utf-8')
    context['form'] = FormPagamento()
    context['valor_a_pagar'] = 100
    context['taxa_reserva'] = '0.1'
    return render(request, 'gere/template1/cliente/apresentacao/teste3.html', context)