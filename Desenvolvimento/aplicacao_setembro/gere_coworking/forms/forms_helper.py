...
def validate(var, tipo):
    ...
    if tipo == 'cnpj_administrador':
        return validate_cnpj(var, isClient=False, isAdm=True)
    if tipo == 'telefone':
...
def validate_cnpj(cnpj, isClient=True, isAdm=False):
    if not isCNPJValid(cnpj):
        raise_error(_("CNPJ inválido"), code='invalid')
    if ClienteModel.objects.is_duplicated(cnpj, tipo_pessoa= "cliente" if isClient else 
    ("administrador" if isAdm else "funcionario")):
